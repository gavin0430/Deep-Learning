...
def telemetry(sid, data):
	if data:
		...
		try:
			...
			'''
			image = np.asarray(image)
			image_f = np.empty([1, 3, width, height])
			image_f[0] = np.transpose(cv2.resize(image,(height,width),interpolation=cv2.INTER_CUBIC), (2, 0, 1))
			image_f = torch.FloatTensor(image_f)
			image_f = image_f.cuda()
			steering_angle = float(model(image_f))
